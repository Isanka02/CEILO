...
export default function AnalyticsDash() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setApiError(err.response?.data?.message || 'Failed to load analytics.');
      } finally {
        setFetching(false);
      }
    })();
  ...
  if (fetching) return (
    <>
      ...
      <div className="min-h-screen flex items-center justify-center" style={{ background: 'var(--cream)' }}>
        <p style={{ fontSize: '0.85rem', color: 'var(--muted)', fontFamily: "'Jost',sans-serif" }}>Loading analytics…</p>
      </div>
    </>
  );
  ...
  if (!data) return (
    <>
      ...
      <Header />
      <div className="min-h-screen flex items-center justify-center" style={{ background: 'var(--cream)' }}>
        <p style={{ fontSize: '0.85rem', color: '#C53030', fontFamily: "'Jost',sans-serif" }}>{apiError || 'Failed to load analytics.'}</p>
      </div>
    </>
  );
        ...
        <div className="max-w-7xl mx-auto px-6 py-12 flex flex-col md:flex-row gap-10">
          ...
          <main className="flex-1 min-w-0">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
              {[
                { label: 'Total Users',    value: data?.totalUsers    ?? 0, icon: 'M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2' },
                { label: 'Total Orders',   value: data?.totalOrders   ?? 0, icon: 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2' },
                { label: 'Total Products', value: data?.totalProducts ?? 0, icon: 'M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10' },
                { label: 'Total Revenue',  value: `LKR ${(data?.totalRevenue ?? 0).toFixed(2)}`, icon: 'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z' },
                ...
                {(data?.recentOrders || []).map(o => {
                  ...
                  return (
                    <div key={o._id} className="table-row" style={{ gridTemplateColumns: '1fr 1fr auto auto', gap: '8px' }}>
                      <div>
                        <p style={{ fontWeight: 500, fontSize: '0.82rem' }}>#{o._id.slice(-8)}</p>
                        <p style={{ fontSize: '0.7rem', color: 'var(--muted)' }}>{fmtDate(o.createdAt)}</p>
                      </div>
                      <span style={{ fontSize: '0.82rem' }}>{o.user?.name || '—'}</span>
                      <span className="status-badge" style={{ background: st.bg, color: st.color }}>
                        <span style={{ width: '5px', height: '5px', borderRadius: '50%', background: st.dot }} />{o.orderStatus}
                      </span>
                      <span style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: '0.95rem', fontWeight: 600, color: 'var(--charcoal)', textAlign: 'right' }}>LKR {(o.totalPrice || 0).toFixed(2)}</span>
                    </div>
                  );
                })}
              </div>
            ...
            <div className="section-card">
              ...
              {(data?.topProducts || []).map((p, i) => (
                <div key={p.name} className="table-row" style={{ gridTemplateColumns: '2fr 1fr 1fr' }}>
                  ...
                  <span style={{ fontSize: '0.85rem', color: 'var(--muted)', textAlign: 'center' }}>{p.totalSold}</span>
                  <span style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: '0.95rem', fontWeight: 600, color: 'var(--charcoal)', textAlign: 'right' }}>—</span>
                </div>
              ))}
            </div>
          </main>
